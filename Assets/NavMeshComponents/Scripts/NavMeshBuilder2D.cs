...
public class NavMeshBuilder2D : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            GetComponent<NavMeshBuilder2D>().RebuildNavmesh(false);
        }
    }
    ...
    public void RebuildNavmesh(bool async)
    {
        ...
        Bounds totalBoundsReversed = new Bounds(Plane2DRotationInverse * totalBounds.center, Plane2DRotationInverse * totalBounds.size); // We need to reverse the rotation that's going to be used for baking to get proper bounds
        Bounds buildBounds = new Bounds(Vector3.zero, Vector3.one * float.MaxValue); //Using enclosing and empty bounds is bugged at the moment - use arbitrarily big one 
        if (!data)
        {
